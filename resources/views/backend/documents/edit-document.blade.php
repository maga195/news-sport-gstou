...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-10 title">
            <h1><i class="fa fa-bars"></i> Изменить документ</h1>
        </div>
        ...
        <div class="col-sm-12">
            <div class="row">
                <form method="post" action="{{ url('updatedocument') }}/{{ $document->did }}" enctype="multipart/form-data">
                    {{ csrf_field() }}
                    <input type="hidden" name="tbl" value="{{ encrypt('documents') }}" />
                    <input type="hidden" name="did" value="{{ $document->did }}" />
                    <div class="col-sm-9">
                        <div class="form-group">
                            <input type="text" name="title" id="post_title" value="{{ $document->title }}" class="form-control" placeholder="Введите название">
                        </div>
                        <div class="form-group">
                            <input type="hidden" name="slug" id="slug" value="{{ $document->slug }}" class="form-control" readonly="">
                        </div>
                        <input type="file" name="image" class="btn btn-primary" value="{{ $document->image }}"/>
                    ...
                    <div class="col-sm-3">
                        <div class="content publish-box">
                            ...
                            <div class="row">
                                <div class="col-sm-12 main-button">
                                    <a href="{{ url('document-article') }}/{{ $document->slug }}"><p>Посмотреть</p></a>
                        ...
                        <div class="content cat-content">
                            ...
                            <p><label for="{{ $cat->dcid }}">
                                    <input type="checkbox" name="category_id[]" value="{{ $cat->dcid }}" @if(in_array($cat->dcid, $documentcat)) checked @endif/>
                                    {{ $cat->title }}</label></p>
