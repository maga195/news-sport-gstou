...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-sm-4 cat-form">
            ...
            <form method="post" action="{{ url('updatecategory') }}/{{ $category->dcid }}">
                {{ csrf_field() }}
                <input type="hidden" name="tbl" value="{{ encrypt('dcategories') }}" />
                <input type="hidden" name="dcid" value="{{ $category->dcid }}" />
            ...
            <div class="content">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <input type="checkbox" name="select-data[]" value="{{ $category->dcid }}">
                                <a href="{{ url('/editdcategory') }}/{{ $category->dcid }}">{{ $category->title }}</a>
                            </td>
                            <td>{{ $category->slug }}</td>
